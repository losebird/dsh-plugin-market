 ...
 *
 * @param hard - If true, break words that exceed width. Default false.
 */
function wrapText(text, width, options) {
    ...
    // Filter out empty lines that result from trailing newlines.
 ...
 */
export function MarkdownTable({ token, highlight, forceWidth, }) {
    ...
    if (useVerticalFormat) {
        return _jsx(Text, { children: renderVerticalFormat() });
    }
    ...
    // Render as a single Text block to prevent Ink from wrapping mid-row.
    // Raw Text (not Ansi) so the fork's span parser doesn't drop SGR 1 bold.
    return _jsx(Text, { children: tableLines.join('\n') });
}
