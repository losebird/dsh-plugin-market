 ...
 */
export function localizedDescription(command) {
 ...
 *   not a command.
 */
export function parseCommandName(line) {
 ...
 */
export function isLocalCommandName(input, list = LOCAL_COMMANDS) {
 ...
 */
export function filterCommands(input, list = LOCAL_COMMANDS) {
    const prefix = input.replace(/^\//, '').trim().toLowerCase();
    return list.filter(command => command.name.toLowerCase().startsWith(prefix));
}
