...
import { cellAt, charInCellAt, diffEach, isEmptyCellAt, shiftRows, visibleCellAtIndex, } from './screen.js';
import { CURSOR_HOME, cursorUp, eraseToEndOfScreen, scrollDown as csiScrollDown, scrollUp as csiScrollUp, RESET_SCROLL_REGION, setScrollRegion, } from './termio/csi.js';
...
export class LogUpdate {
    ...
    render(prev, next, altScreen = false, decstbmSafe = true) {
        ...
        if (prevHadScrollback && nextFitsViewport && isShrinking) {
            logForDebugging(`Viewport repaint (shrink->below): prevHeight=${prev.screen.height}, nextHeight=${next.screen.height}, viewport=${prev.viewport.height}`);
            return repaintViewportInPlace(prev, next, stylePool);
        }
        ...
        if (shrinking) {
            ...
            if (!altScreen && prev.screen.height > prev.viewport.height) {
                if (cursorAtBottom) {
                    logForDebugging(`Viewport repaint (shrink tall): prevHeight=${prev.screen.height}, nextHeight=${next.screen.height}, viewport=${prev.viewport.height}`);
 ...
 */
function repaintViewportInPlace(prev, next, stylePool) {
