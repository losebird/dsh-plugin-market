...
export function ThemeProvider({ children, theme, }) {
    ...
    const [forcedValid] = useState(() => {
        ...
        if (isThemeAvailable(forced))
            return true;
        console.warn(`[dsh-tui] theme "${forced}" not found (built-ins: light, dark, dark-ansi; user themes: ~/.dsh-cc/themes/*.json); falling back to auto-detection`);
        return false;
    });
    const [active, setActive] = useState(forcedValid ? forced ?? null : null);
    ...
    const value = React.useMemo(() => ({ theme: active ?? 'dark', setTheme }), [active, setTheme]);
    useEffect(() => {
        if (active !== null)
