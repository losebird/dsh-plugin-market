...
// Official site still imports src/install-info.mjs (Vite). Keep the helpers below in sync with that file.
...
function isKitchenCommand(text) {
  const s = String(text || '')
  if (!s) return false
...
function asPluginAddSpec(value) {
  const s = str(value)
...
function officialDownloadUrl(item) {
  if (!item || typeof item !== 'object') return null
...
function installPresentation(item) {
